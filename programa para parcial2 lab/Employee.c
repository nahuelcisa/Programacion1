...
int mostrarEmpleadoEspecifico(LinkedList* lista){
    ...
    if( printf("Id   Nombre   HorasTrabajadas   Sueldo\n\n") &&
       mostrarEmpleado(aux->pElement)){
            printf("Esta persona se encuentra en la posicion %d de la lista\n\n",ll_indexOf(lista,aux->pElement));
            todoOk = 1;
       }
...
int mostrarEmpleadoEspecificoYelimina(LinkedList* lista){
    ...
    if(lista != NULL){
            ...
            scanf("%d",&indice);
            while(indice> ll_len(lista) || indice < 0){
            printf("Error. El indice no puede ser menor a cero o mayor a los elementos que tiene la lista cargados. Reingrese:");
            scanf("%d",&indice);
        }

        muestras = test_getNode(lista,indice -1);
        mostrarEmpleado(muestras->pElement);
        ll_pop(lista,indice-1);
